...
def load_metrics_data():
    ...
    Retorna un DataFrame con los datos seleccionados.
    """
    # Campos que queremos incluir en el DataFrame
    FIELDS_TO_INCLUDE = [
    ...
    for file in metrics_dir.glob("TestToolAgent_*.json"):
        try:
            with open(file, 'r') as f:
                ...
                filtered_data = {k: data.get(k) for k in FIELDS_TO_INCLUDE}
...
# El return se hace mediante diccionario para facilitar la lectura y recuperación de los datos.
...
def calculate_retriever_metrics(df: pd.DataFrame) -> dict:
    ...
    Retorna un diccionario con el promedio de vectores recuperados.
    """
    return {
